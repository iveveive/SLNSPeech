...
class BaseDataset(torchdata.Dataset):
    ...
    def _init_vtransform1(self):
        transform_list = []
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]

        if self.split == 'train':
            transform_list.append(vtransforms.Resize(int(self.signSize * 1.1), Image.BICUBIC))
            transform_list.append(vtransforms.RandomCrop(self.signSize))
            transform_list.append(vtransforms.RandomHorizontalFlip())
        else:
            transform_list.append(vtransforms.Resize(self.signSize, Image.BICUBIC))
            transform_list.append(vtransforms.CenterCrop(self.signSize))

        transform_list.append(vtransforms.ToTensor())
        transform_list.append(vtransforms.Normalize(mean, std))
        transform_list.append(vtransforms.Stack())
        self.vid_transform1 = transforms.Compose(transform_list)
    ...
    def _load_signs(self, paths):
        signs = []
        for path in paths:
            signs.append(self._load_frame(path))
        signs = self.vid_transform1(signs)
        return signs
